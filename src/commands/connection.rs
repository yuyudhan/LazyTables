    ...
    fn execute(&self, context: &mut CommandContext) -> Result<CommandResult> {
        ...
        if !context.state.db.connections.connections.is_empty() {
            ...
            context
                .state
                .toast_manager
                .info(format!("Connecting to database #{selected}"));

            Ok(CommandResult::SuccessWithMessage(format!(
                "Initiated connection to database #{selected}"
            )))
        } else {
    ...
    fn execute(&self, context: &mut CommandContext) -> Result<CommandResult> {
        if !context.state.db.connections.connections.is_empty() {
            ...
            context
                .state
                .toast_manager
                .info(format!("Testing connection #{selected}"));
