...
use super::{Command, CommandCategory, CommandContext, CommandId, CommandResult, CommandAction};
...
impl Command for HelpCommand {
    fn execute(&self, context: &mut CommandContext) -> Result<CommandResult> {
        ...
        Ok(CommandResult::SuccessWithMessage("Help overlay opened".to_string()))
    }
    ...
    fn execute(&self, context: &mut CommandContext) -> Result<CommandResult> {
        ...
        match context.state.focused_pane {
                ...
                if !context.state.query_content.is_empty() {
                    let query = &context.state.query_content;
                        ...
                        context.state.toast_manager.success(
                            &format!("Query saved to {}", filename)
                        );
                        
                        Ok(CommandResult::SuccessWithMessage(
                            format!("Query saved to {}", filepath.display())
                        ))
                } else {
            ...
            crate::app::FocusedPane::TabularOutput => {
                ...
                Ok(CommandResult::Error("Export not yet implemented".to_string()))
            }
            _ => {
                Ok(CommandResult::Error("Nothing to save in current pane".to_string()))
            }
        }
    }
...
impl Command for OpenCommand {
    fn execute(&self, context: &mut CommandContext) -> Result<CommandResult> {
        match context.state.focused_pane {
            crate::app::FocusedPane::SqlFiles => {
                // Load selected SQL file
                {
                    ...
                    if selected < files.len() {
                        ...
                        Ok(CommandResult::SuccessWithMessage(
                            format!("Loaded {}", files[selected].file_name().to_string_lossy())
                        ))
                    } else {
            ...
            _ => {
                Ok(CommandResult::Error("Navigate to SQL Files pane to open a file".to_string()))
            }
        }
    }
