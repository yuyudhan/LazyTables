...
pub trait Command: Send + Sync {
    ...
    fn undo(&self, _context: &mut CommandContext) -> Result<CommandResult> {
        Ok(CommandResult::Error("Undo not supported for this command".to_string()))
    }
...
impl CommandRegistry {
    ...
    pub fn execute(&mut self, id: CommandId, context: &mut CommandContext) -> Result<CommandResult> {
        if let Some(command) = self.commands.get(&id) {
            // Check if command can be executed
            if !command.can_execute(context) {
                return Ok(CommandResult::Error("Command cannot be executed in current context".to_string()));
            }
    ...
    pub fn undo(&mut self, context: &mut CommandContext) -> Result<CommandResult> {
        if let Some(id) = self.undo_stack.pop() {
            if let Some(command) = self.commands.get(&id) {
                ...
                Ok(CommandResult::Error("Command not found for undo".to_string()))
            }
        } else {
    ...
    pub fn redo(&mut self, context: &mut CommandContext) -> Result<CommandResult> {
        if let Some(id) = self.redo_stack.pop() {
            if let Some(command) = self.commands.get(&id) {
                ...
                Ok(CommandResult::Error("Command not found for redo".to_string()))
            }
        } else {
